  ...
  return (
    <>
      <div className="lg:hidden fixed z-30 -bottom-0.5 flex items-center justify-between shadow-bottomNavigation body-font bg-skin-fill w-full h-14 px-4 md:px-6 lg:px-8 text-skin-muted pb-0.5 dark:bg-darkBg border-t-[0.5px] border-borderSeperator">
        <Link
          // href={`/r/${restaurant ? restaurant : ''}/${table ? table : ''}`}
          // href={`${restaurant ? `/r/${restaurant}/${table}` : '/'}`}
          // href={link}
        ...
        >
          <MenuIcon className="dark:text-darkText" />
        </button> */}
        <button
        ...
        {/* <ShoppingBagIcon className="h-6 dark:text-darkText" /> */}
        ...
        <AuthMenu
          ...
          btnProps={{
            ...
            children: <UserCircleIcon className="h-6 dark:text-darkText" />,
            onClick: handleLogin,
          }}
        >
          <UserCircleIcon className="h-6 dark:text-darkText" />
        </AuthMenu>
      </div>
      <Drawer
